...
//    }
    ...
    public ChatRoom createChatRoom(Long itemId, Member member) {
        Item item = itemRepository.findItemById(itemId);
        String name = member.getNickname() + "님의 " + item.getShop().getShopName() + " 상점의 " + item.getName() + " 문의 ";
        ChatRoom chatRoom = ChatRoom.create(name, item, member, item.getShop().getMember());
     ...
     */
    public ChatRoomResponseDto enterChatRoom(String roomId, Member member) {
        log.info("채팅방 들어옴");
    ...
    public List<ChatRoom> getSellerEnterRooms(Member seller) {
